...
讓 workflow 依工作需要的判斷成本自動選 model。固定的一個指令一個動作、普通
工具、機械式多步驟與尚未證明複雜的工作走 cheap Luna；一般設計、工具選擇、
輸出解讀、QA 與 bounded implementation 走 Sol；只有高信心的深層架構、跨系統
權衡、衝突證據或進階工具編排才自動升級 Astra。Miyago 不需要在 prompt 中
要求「開多 role」。

## Scope

- 在 `UserPromptSubmit` 由 hook 判定啟動時機與實現目的，產生保守、redacted
  的 atomic/guarded/judgment/deep_judgment route signal。
- route signal 明確帶出 escalation 條件與 dispatch contract，讓 parent 不必
  猜測是否要開 role、使用哪個 task 或 fork 邊界。
- 一般 judgment turn 自動要求 `sol-executor` typed role，綁定
  `gpt-5.6-sol@high`；deep judgment 才要求 Astra `executor`。
...
- 若 judgment 或 deep judgment turn 結束時沒有對應的 typed child，透過既有
  Stop hook 依 pending role 最多自動重試一次。
...
普通工具操作、機械式多步驟、需求含糊或 atomic 判定不確定時成立。
parent 使用 cheap Luna 做 bounded probe，不因單一關鍵字或不確定本身建立
child；只有 probe 產生 deep complexity evidence 才升級。

### `judgment`

一般設計、工具選擇、輸出解讀、QA、跨檔案判斷或 bounded implementation 需求
成立。route 是 `sol-executor`，由 `gpt-5.6-sol@high` 負責局部設計與執行；若
遇到真正的 deep boundary，回報 evidence 並升級到 `executor`。
...
未列出的動作時，先交回 Sol；Sol 仍遇到 deep boundary 才升級 Astra。hook 只
自動重試一次，避免錯誤路徑造成無限 token 消耗。

## Acceptance

- 明確的單一 command 產生 `atomic` signal，且不要求 child。
- 普通工具、模糊需求與不確定 prompt 產生 `guarded` signal，保持 cheap
  parent-local execution。
- 一般設計、工具選擇、輸出解讀、QA 或 bounded implementation 產生 `judgment`
  signal，且要求 `sol-executor` 與 `gpt-5.6-sol@high`。
- 高信心 deep architecture、跨系統權衡、衝突證據或進階工具編排才產生
  `deep_judgment` signal，且要求 `executor` 與 `gpt-6-astra@high`。
- judgment turn 沒有 typed child 時，Stop hook 依 marker 的 `required_role`
  只重試一次；directive 固定 `task_name=automatic_model_route` 與
  `fork_turns=none`。Codex 已由 Stop hook 續行後，不得再次鎖住同一 session。
- `verifier`、`plan-verifier`、`security-reviewer` 與 `security-executor` 維持
  `gpt-5.6-sol@high`；`mech-executor` 與 `scout` 維持 cheap Luna binding。
...
- [x] 將 Luna／Sol／Astra 三層 role binding 與 policy route 接上。
